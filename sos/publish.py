...
call here is the existing, unmodified sos/charts.py logic — only the sizing
presets and output shape (dict instead of rendered chart) are new.
"""
...
from .presets import NEXTN_KWARGS, SCATTER_KWARGS, SEASON_KWARGS, VARIANTS, NEXT_N_VALUES
...
def build_round_artifacts(
    ...
    for n in NEXT_N_VALUES:
        ...
        for variant in VARIANTS:
            chart = build_nextN_altair_logos_table(
                ...
                **NEXTN_KWARGS[variant],
            )
            artifacts[f"rounds/{round_num}/next-n/{n}/{variant}.json"] = chart.to_dict()
    ...
    for variant in VARIANTS:
        main_chart, table_chart = make_sos_scatter_and_side_table(
            ...
            **SCATTER_KWARGS[variant],
        )
        artifacts[f"rounds/{round_num}/scatter/{variant}.json"] = {
            "main": main_chart.to_dict(),
            "table": table_chart.to_dict(),
        }

    for variant in VARIANTS:
        chart = make_sos_table_chart(
            sos_net=sos_net,
            sos_win=sos_win,
            team_to_logo_path=team_to_logo_path,
            logo_to_dataurl=logo_to_dataurl,
            ...
            season_label=season_label,
            mobile_mode=(variant == "mobile"),
            **SEASON_KWARGS[variant],
        )
        artifacts[f"rounds/{round_num}/season-table/{variant}.json"] = chart.to_dict()
