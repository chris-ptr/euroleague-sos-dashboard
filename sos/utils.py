...
def logo_to_dataurl(path: str | None) -> str | None:
    ...
    logo_to_site_url instead — see the note there.
    """
...
def logo_to_site_url(path: str | None) -> str | None:
    """
    Resolve a local logo path to the site-relative URL the published charts use.
    ...
    Root-relative, so the specs stay portable across hosts and previews — the
    PNGs are served from the same origin as the page.
    """
    if not path or not isinstance(path, str):
        return None
    return f"{config.PUBLISH_URL_PREFIX}/logos/{quote(os.path.basename(path))}"
